 ...
 * Використовується callback DrawPixelFunc, що може бути користувацькою функцією малювання пікселя.
 */
 ...
 * DrawPixelFunc - callback для малювання пікселя.
 */
...
void DrawTextWithBackground(const RasterFont font, int x, int y, const char* text,
    ...
    for(int i = 0; i < borderThickness; i++) {
        DrawRect(x - padding - borderThickness + i, y - padding - borderThickness + i,
    ...
    int ypos = y;
    for (int i = 0; i < lineCount; i++) {
        DrawTextScaled(font, x, ypos, lines[i], spacing, scale, textColor, DrawPixel);
